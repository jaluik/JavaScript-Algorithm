...
function mix(s1, s2) {
  // your code
  let obj1 = generateObj(s1);
  let obj2 = generateObj(s2);
  let array = [];
  for (let key1 in obj1) {
    if (obj2[key1]) {
      if (obj2[key1] > obj1[key1]) {
        array.push("2:" + key1.repeat(obj2[key1]));
      } else if (obj2[key1] === obj1[key1]) {
        array.push("=:" + key1.repeat(obj2[key1]));
      } else {
        array.push("1:" + key1.repeat(obj1[key1]));
      }
    } else {
      array.push("1:" + key1.repeat(obj1[key1]));
    }
  ...
  for (let key2 in obj2) {
    if (!obj1[key2]) {
      array.push("2:" + key2.repeat(obj2[key2]));
    }
  }
  array.sort((a, b) => {
